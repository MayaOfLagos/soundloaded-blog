...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = `Nigerian entertainment gist, celebrity news, and industry gossip on ${s.siteName}.`;
  return {
    title: "Gist",
    description,
    alternates: { canonical: "/gist" },
    openGraph: {
      title: `Gist | ${s.siteName}`,
      description,
...
export default async function GistPage() {
  ...
  if (!settings.enableGist) return <SectionDisabled section="Gist" />;
  const schema = buildCollectionPageSchema(
    "Gist",
    `Entertainment gist and celebrity news on ${settings.siteName}.`,
    "/gist",
    settings.siteUrl,
    settings.siteName
  );
          ...
          <main className="min-w-0">
            <section className="mb-6">
              <Suspense fallback={<HeroSkeleton />}>
                <CategoryHero type="GIST" emptyMessage="No featured gist yet. Check back soon!" />
              </Suspense>
            </section>
