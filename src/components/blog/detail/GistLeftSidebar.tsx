...
import { Flame, BarChart3, Sparkles, FolderOpen } from "lucide-react";
import {
  getTrendingGistPosts,
  getHotGistPosts,
  getGistCategories,
  getMixedContent,
} from "@/lib/api/posts";
...
interface GistLeftSidebarProps {
  excludePostId?: string;
}

export async function GistLeftSidebar({ excludePostId }: GistLeftSidebarProps) {
  const settings = await getSettings();
  const ps = settings.permalinkStructure;

  const [trending, hot, categories, mixed] = await Promise.all([
    getTrendingGistPosts({ limit: 5, excludeId: excludePostId, permalinkStructure: ps }),
    getHotGistPosts({ limit: 5, excludeId: excludePostId, permalinkStructure: ps }),
    getGistCategories(),
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Hot Gist — This Week */}
      <SidebarSection
        icon={<Flame className="text-brand h-3.5 w-3.5" />}
        title="Hot Gist"
        posts={hot}
      />
      ...
      {/* Mixed Content — Other Categories */}
      <SidebarSection
