...
import { useUserFavorites } from "@/hooks/useUserDashboard";
...
export function FavoritesView() {
  ...
  const { data, isLoading } = useUserFavorites(page, type || undefined, sort);
  const toggleFavorite = useToggleFavorite();

  const handleRemove = (favoriteId: string) => {
    toggleFavorite.mutate({ favoriteId });
  };
      ...
      {favorites.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Mark your favorite articles and music to find them quickly.
          </p>
