...
function ToolbarButton({
  ...
  if (blur) {
    return (
      <button className="hover:bg-accent/50 rounded-md p-1 transition-colors hover:cursor-pointer">
        <motion.div
...
function ExtendedToolbar() {
  ...
  return (
    <motion.div
      className="bg-muted border-border relative h-14 overflow-hidden rounded-full border"
      initial={{ width: initialWidth }}
      animate={hasMeasurements ? { width: currentWidth } : { width: initialWidth }}
    ...
    >
      ...
      >
        ...
        <div
          ...
          className="flex flex-shrink-0 items-center gap-1 p-1.5 pr-2 pl-3"
        >
          {primaryTools.map((item, index) => (
            <ToolbarButton key={index} icon={item.icon} blur={item.blur} isBlurred={isExpanded} />
          ))}
          <motion.button
            whileTap={{ scale: 0.9 }}
            onClick={() => setIsExpanded(true)}
            className="bg-background flex aspect-square h-full items-center justify-center rounded-full"
          >
            <HugeiconsIcon
        ...
        <div
          ...
          className="flex flex-shrink-0 items-center gap-1 p-1.5 pr-3 pl-2"
          style={{
